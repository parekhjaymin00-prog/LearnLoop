...
export function AuthForm({ onLoginSuccess }: { onLoginSuccess?: () => void }) {
    ...
    return (
        <Card className="w-full max-w-[480px] shadow-xl border-border/50">
            <CardHeader className="text-center space-y-1">
                <CardTitle className="text-2xl font-bold">Join the Community</CardTitle>
                <CardDescription>
                    Start your learning journey with LearnLoop
                </CardDescription>
            </CardHeader>
            <CardContent>
                {/* Segmented Control */}
                <div className="flex p-1 mb-6 bg-muted rounded-lg">
                    <button
                        type="button"
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    ...
                    {!isLogin && (
                        <div className="space-y-2">
                            ...
                            <Input id="fullname" name="fullname" placeholder="John Doe" autoComplete="name" required />
                        </div>
                    )}
                    ...
                    <div className="space-y-2">
                        ...
                        <Input id="password" name="password" type="password" placeholder="••••••••" autoComplete={isLogin ? "current-password" : "new-password"} required />
                        {!isLogin && (
                            <p className="text-xs text-muted-foreground">At least 8 characters with numbers and letters</p>
                        )}
                    </div>
            ...
            <CardFooter>
                <div className="w-full text-center text-sm text-muted-foreground">
                    {isLogin ? (
                        <Link href="#" className="hover:text-primary underline underline-offset-4">Forgot your password?</Link>
                    ) : (
                        <p>Already have an account? <Link href="#" className="text-primary hover:underline" onClick={(e) => { e.preventDefault(); setIsLogin(true); }}>Log In</Link></p>
                    )}
                </div>
            </CardFooter>
        </Card>
